...
JSBool js_ls_Leafsoar_functionTest(JSContext *cx, uint32_t argc, jsval *vp)
{
	...
	JSObject *obj = NULL;
    ls::AdBridge* cobj = NULL;
	obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	cobj = (ls::AdBridge *)(proxy ? proxy->ptr : NULL);
...
JSBool js_ls_Leafsoar_constructor(JSContext *cx, uint32_t argc, jsval *vp)
{
	...
	if (argc == 0) {
        ...
        ls::AdBridge* cobj = new ls::AdBridge();
        cocos2d::CCObject* _ccobj = dynamic_cast<cocos2d::CCObject*>(cobj);
        if (_ccobj){
            _ccobj->autorelease();
        }
        TypeTest<ls::AdBridge> t;
...
JSBool js_ls_Leafsoar_create(JSContext *cx, uint32_t argc, jsval *vp)
{
	...
	if (argc == 0) {
        ls::AdBridge* ret = ls::AdBridge::create();
		jsval jsret;
		do {
            if (ret) {
                js_proxy_t *proxy = js_get_or_create_proxy<ls::AdBridge>(cx, ret);
...
void js_register_ls_Leafsoar(JSContext *cx, JSObject *global) {
	...
	jsb_LsLeafsoar_class->name = "AdBridge";
	...
	static JSFunctionSpec funcs[] = {
		JS_FN("showAdAtTop", js_ls_Leafsoar_functionTest, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
	};
	...
	JSBool found;
	JS_SetPropertyAttributes(cx, global, "AdBridge", JSPROP_ENUMERATE | JSPROP_READONLY, &found);
